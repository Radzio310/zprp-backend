...
from app.mentoring_rules import pair_matches, season_bounds
...
async def enqueue(event_id, previous_state=None):
    ...
    for link in links:
        ...
        if link["mentor_id"] in (event["target_judge_ids"] or []):
            continue
    ...
    for device in devices:
        data = {**dict(event["data_json"] or {}), "kind": "mentoring_match_change", "mentoring_pair_ids": recipients[device["judge_id"]], "mentoring_mentor_id": device["judge_id"]}
...
async def delivery_allowed(data, token_row):
    ...
    if not token_row or token_row["judge_id"] != mentor_id:
        return False
    return bool(await database.fetch_one(select(assignments.c.pair_id).select_from(assignments.join(pairs, pairs.c.id == assignments.c.pair_id)).where(assignments.c.pair_id.in_(data.get("mentoring_pair_ids") or [])).where(assignments.c.mentor_id == mentor_id).where(assignments.c.ended_at.is_(None)).where(assignments.c.notify.is_(True)).where(pairs.c.ended_at.is_(None))))
